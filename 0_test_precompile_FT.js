...
async function main() {
	...
	const allowanceApproveFtRx = await contracts.executeContractFcn(contractId, "approveFt", allowanceApproveFtParams, gasLim, client);
	client.setOperator(operatorId, operatorKey);

	console.log(`- Contract call for FT allowance approval (check in mirror node ↓): ${allowanceApproveFtRx.status}`);
	...
	const allowanceSendFtRx = await contracts.executeContractFcn(contractId, "ftTransferApproved", allowanceSendFtParams, gasLim, client);
	client.setOperator(operatorId, operatorKey);
	console.log(`- Contract call for approved FT transfer: ${allowanceSendFtRx.status} \n`);
	...
	client.setOperator(treasuryId, treasuryKey);
	const allowanceDeleteFtRx = await contracts.executeContractFcn(contractId, "approveFt", allowanceDeleteFtParams, gasLim, client);
	client.setOperator(operatorId, operatorKey);

	console.log(`- Contract call for FT allowance deletion (check in mirror node ↓): ${allowanceDeleteFtRx.status}`);
	console.log(`- https://testnet.mirrornode.hedera.com/api/v1/accounts/${treasuryId}/allowances/tokens`);
